...
export class ControladorCatDptoRegion {
  constructor(private readonly casoUso: CasoUsoDptoRegion) {}
  ...
  public obtenerRegion = async (req: Request, res: Response) => {
   
  
    try {
      const resultado = await this.casoUso.obtenerRegionPais();
      res.send(resultado);
    } catch (e) {
      handleHttp(res, ' ' + e);
    }
