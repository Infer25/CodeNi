...
export class ControladorCatRubro {
  constructor(private readonly casoUso: CasoUsoCatRubro) {}
  ...
  public obtenerTipoRubro = async (req: Request, res: Response) => {
   
  
    try {
      const resultado = await this.casoUso.obtenerTipoRubro();
      res.send(resultado);
    } catch (e) {
      handleHttp(res, ' ' + e);
    }
