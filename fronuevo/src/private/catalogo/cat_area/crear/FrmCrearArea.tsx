...
import { useCreatearea } from "../query";
...
export const FrmCreararea: React.FC<props> = ({
  state,
  setStateCrear,
}) => {
  const theme = useTheme();
  const celular = useMediaQuery(theme.breakpoints.down("sm"));
  const { mutate: crear } = useCreatearea();
             ...
             defaultValue=""
              render={({ field: { onChange, value } }) => (
                <TextField
                  ...
                  autoComplete="off"
                  placeholder="Escriba la descripcion del area"
                  multiline
                  rows={celular ? 12 : 10}
                />
              )}
