...
import { useCreatecategoria_proveedor } from "../query";

type props = {
  state: boolean;
  setStateCrear: React.Dispatch<React.SetStateAction<boolean>>;
};
export const FrmCrearcategoria_proveedor: React.FC<props> = ({
  state,
  setStateCrear,
}) => {
  const theme = useTheme();
  const celular = useMediaQuery(theme.breakpoints.down("sm"));
  const { mutate: crear } = useCreatecategoria_proveedor();
    ...
    >
      <FormContainer>
        <ToolBarTitle
          ...
          subMenu="Registro de categoria-proveedor"
          ...
          >
            <Controller
              ...
              render={({ field: { onChange, value } }) => (
                <TextField
                  id="categoria_proveedor"
                  label="categoria proveedor"
                  size="medium"
                  //fullWidth
                  onChange={onChange}
                  value={value}
                  ...
                  type="search"
                  autoComplete="off"
                  placeholder="Escriba el nombre del categoria_proveedor"
                />
              )}
            />
            ...
            <Controller
              ...
              render={({ field: { onChange, value } }) => (
                <TextField
                  ...
                  autoComplete="off"
                  placeholder="Escriba la descripcion del categoria_proveedor"
                  multiline
                  rows={celular ? 12 : 10}
                />
              )}
