...
import { useCreateCualidad } from "../query";

export const FrmCrearCualidad: React.FC<{}> = () => {
  const theme = useTheme();
  const celular = useMediaQuery(theme.breakpoints.down("sm"));
  const { mutate: crear } = useCreateCualidad();
  const {
    handleSubmit,
    control,
    formState: { errors },
  ...
  return (
    <FormContainer>
      <ToolBarTitle
        ...
        subMenu="Registro de cualidad"
        children={
          <CerrarBtn
              ruta={`/${PrivateRoutes.PRIVATE}/${PrivateRoutes.SISTEMA}/${PrivateRoutes.CATCUALIDAD}`}
            />}
      />
      <Box
        component={"form"}
          ...
          <Controller
            ...
            render={({ field: { onChange, value } }) => (
              <TextField
                ...
                type="search"
                autoComplete="off"
                placeholder="Escriba el nombre de la cualidad"
              />
            )}
          />
          ...
          <Controller
            ...
            render={({ field: { onChange, value } }) => (
              <TextField
                ...
                type="search"
                autoComplete="off"
                placeholder="Escriba la descripcion de la cualidad"
                multiline
                rows={celular ? 5 : 10}
              />
            )}
