...
export const TableBodyFrmEstadoCivil: React.FC<ModelTable> = ({ List }) => {
  const theme = useTheme();
  const celular = useMediaQuery(theme.breakpoints.down("lg"));
  const navigate = useNavigate();

  const setValueUpdate = useStoreGlobal((state) => state.setValueFrmEstadoCivilActualizar);
      ...
      {List.map((x) => (
        <TableRow key={x.id}>
          <StyledTableCell
            component="th"
            scope="row"
            data-titulo={celular ? `${"Estado civil"} :  ` : ""}
          >
            {x.Nombre}
          </StyledTableCell>
