...
import { ModelFrmforma_pagoFormulario } from "@/shared/zustand/slice/frmCatFormaPago";
import { FormProvider, useForm } from "react-hook-form";
import { FrmActualizarforma_pago } from "../actualizar/FrmActualizarforma_pago";
import { FrmCrearforma_pago } from "../crear/FrmCrearforma_pago";
import { useFetchGetAllforma_pago } from "../query";
import { ListCriterioforma_pago } from "./criterio";
import { Itemforma_pagoColumna } from "./forma_pagoHeader";
import { TableBodyFrmforma_pago } from "./tableBody";
import { PrivateRoutes } from "@/routes/routes";
import { CerrarBtn } from "@/shared/components/cerrar";

export const FrmListarforma_pago: React.FC<{}> = () => {
  ...
  const handleChangeCriterio = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    switch ((event.target as HTMLInputElement).value) {
      case "nombre": {
        setPlaceholder({
          label: "forma_pago",
          placeHolder: "Filtrar por forma_pago",
        });
        break;
      }
  ...
  const methods = useForm<ModelFrmforma_pagoFormulario>();

  const { data } = useFetchGetAllforma_pago(
    valueRadio,
    valorFiltro,
    pagina,
    comboValue.value
    ...
    <FormProvider {...methods}>
      <FormContainer>
        <ToolBarTitle
          ...
          subMenu="Lista de forma de pago"
        ...
        <BuscadorBoton
          ...
          criterioFiltro={
            <RadioGrupCriterio
              ...
              itemList={ListCriterioforma_pago}
            />
          }
        ...
        <TableCustom
          list={Itemforma_pagoColumna}
          children={
            <TableBodyFrmforma_pago
      ...
      <FrmCrearforma_pago state={stateCrear} setStateCrear={setStateCrear} />
      <FrmActualizarforma_pago
        state={stateUpdate}
