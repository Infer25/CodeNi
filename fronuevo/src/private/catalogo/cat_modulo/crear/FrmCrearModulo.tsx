...
import { useCreatemodulo } from "../query";

type props = {
  state: boolean;
  setStateCrear: React.Dispatch<React.SetStateAction<boolean>>;
};
export const FrmCrearmodulo: React.FC<props> = ({ state, setStateCrear }) => {
  const theme = useTheme();
  const celular = useMediaQuery(theme.breakpoints.down("sm"));
  const { mutate: crear } = useCreatemodulo();
            ...
            <Controller
              ...
              render={({ field: { onChange, value } }) => (
                <TextField
                  ...
                  type="search"
                  autoComplete="off"
                  placeholder="Escriba el nombre del modulo"
                />
              )}
            />
            ...
            <Controller
              ...
              render={({ field: { onChange, value } }) => (
                <TextField
                  ...
                  autoComplete="off"
                  placeholder="Escriba la descripcion del modulo"
                  multiline
                  rows={celular ? 12 : 10}
                />
              )}
