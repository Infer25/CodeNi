...
export const FrmActualizarMunicipio: React.FC<{}> = () => {
  ...
  const onSubmit = (data: ModelFrmActualizarMunicipio) => {
    if (data.nombre != "" && data.descripcion != "" && data.id != 0) {
      actualizar(data);
      reset();
      navigate(  `/${PrivateRoutes.PRIVATE}/${PrivateRoutes.SISTEMA}/${PrivateRoutes.CATMUNICIPIO}`);
    } else {
      alert("no valido");
    }
  };
  ...
  return (
    <FormContainer>
      <ToolBarTitle
        ...
        subMenu="Actualizacion de  municipio"
        children={
          <CerrarBtn
            ruta={`/${PrivateRoutes.PRIVATE}/${PrivateRoutes.SISTEMA}/${PrivateRoutes.CATMUNICIPIO}`}
          />
        }
      />
      <Box
