...
import { useCreatepermiso } from "../query";
...
export const FrmCrearpermiso: React.FC<props> = ({
  state,
  setStateCrear,
}) => {
  const theme = useTheme();
  const celular = useMediaQuery(theme.breakpoints.down("sm"));
  const { mutate: crear } = useCreatepermiso();
             ...
             defaultValue=""
              render={({ field: { onChange, value } }) => (
                <TextField
                  ...
                  autoComplete="off"
                  placeholder="Escriba la descripcion del permiso"
                  multiline
                  rows={celular ? 12 : 10}
                />
              )}
