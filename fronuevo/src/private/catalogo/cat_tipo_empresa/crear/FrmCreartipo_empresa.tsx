...
import { useCreatetipo_empresa } from "../query";

type props = {
  state: boolean;
  setStateCrear: React.Dispatch<React.SetStateAction<boolean>>;
};
export const FrmCreartipo_empresa: React.FC<props> = ({
  state,
  setStateCrear,
}) => {
  const theme = useTheme();
  const celular = useMediaQuery(theme.breakpoints.down("sm"));
  const { mutate: crear } = useCreatetipo_empresa();
    ...
    >
      <FormContainer>
        <ToolBarTitle
          ...
          subMenu="Registro de tipo_empresa"
            ...
            <Controller
              ...
              render={({ field: { onChange, value } }) => (
                <TextField
                  ...
                  type="search"
                  autoComplete="off"
                  placeholder="Escriba el nombre del tipo_empresa"
                />
              )}
            />
            ...
            <Controller
              ...
              render={({ field: { onChange, value } }) => (
                <TextField
                  ...
                  autoComplete="off"
                  placeholder="Escriba la descripcion del tipo_empresa"
                  multiline
                  rows={celular ? 12 : 10}
                />
              )}
