...
export type ModelTableRowFrmTipoRubro = {
  id: string;
  Nombre: string;
  Descripciòn: string;
  Registrado: string;
...
export type ModelTable = {
  List: ModelTableRowFrmTipoRubro[];
};
export const TableBodyFrmTipoRubro: React.FC<ModelTable> = ({ List }) => {
  const theme = useTheme();
  const celular = useMediaQuery(theme.breakpoints.down("lg"));
  const navigate = useNavigate();

  const setValueUpdate = useStoreGlobal((state) => state.setValueFrmTipoRubro);
