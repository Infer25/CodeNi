...
import { useFetchGetAllColor } from "../query";
import { rutaFrmColor } from "./ruta";
import { ListCriterioColor } from "./criterio";
import { ItemColorColumna } from "./colorHeader";
import { TableBodyFrmColor } from "./tableBody";

export const FrmListarColor: React.FC<{}> = () => {
  ...
  const handleChangeCriterio = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    switch ((event.target as HTMLInputElement).value) {
      case "nombre": {
        setPlaceholder({ label: "Color", placeHolder: "Filtrar por color" });
        break;
      }
  ...
  const { data } = useFetchGetAllColor(
    valueRadio,
    valorFiltro,
    pagina,
    comboValue.value
  ...
  return (
    <FormContainer>
      ...
      <TableCustom
        list={ItemColorColumna}
        children={<TableBodyFrmColor List={data?.rows ? data?.rows : []} />}
      />
