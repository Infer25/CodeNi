...
import { rutaFrmActualizarDptoRegion } from ".";
...
export const FrmActualizarDptoRegion: React.FC<{}> = () => {
  ...
  const { id, nombre, descripcion ,num_region,Region} = useStoreGlobal((state) => ({
  ...
  const onSubmit = (data: ModelFrmActualizarDptoRegion) => {
    if (data.nombre != "" && data.descripcion != "" && data.id != 0) {
      actualizar(data);
      reset();
      navigate("/sistema/viewCatalogo/dptoRegion");
    } else {
      alert("no valido");
    }
  };
  ...
  return (
    <FormContainer>
      <ToolBarTitle
        ...
        url={rutaFrmActualizarDptoRegion}
      />
      <Box
        component={"form"}
