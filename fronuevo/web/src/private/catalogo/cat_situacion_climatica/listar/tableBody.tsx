...
export const TableBodyFrmSituacionClimatica: React.FC<ModelTable> = ({ List }) => {
  const theme = useTheme();
  const celular = useMediaQuery(theme.breakpoints.down("lg"));
  const navigate = useNavigate();

  const setValueUpdate = useStoreGlobal((state) => state.setValueFrmSituacionClimaticaActualizar);
      ...
      {List.map((x) => (
        <TableRow key={x.id}>
          <StyledTableCell
            component="th"
            scope="row"
            data-titulo={celular ? `${"Situacion climatica"} :  ` : ""}
          >
            {x.Nombre}
          </StyledTableCell>
