...
import { TableBodyFrmTemporada } from "./tableBody";

import { ItemTemporadaColumna, ListCriterioTemporada } from ".";
import { useFetchGetAllTemporada } from "../query";
import { rutaFrmCualidad } from "./ruta";



export const FrmListarTemporada: React.FC<{}> = () => {
  ...
  const handleChangeCriterio = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    switch ((event.target as HTMLInputElement).value) {
      case "nombre": {
        setPlaceholder({ label: "Temporada", placeHolder: "Filtrar por temporada" });
        break;
      }
  ...
  const { data } = useFetchGetAllTemporada(
    valueRadio,
    valorFiltro,
    pagina,
    comboValue.value
  ...
  return (
    <FormContainer>
      <ToolBarTitle
        ...
        subMenu="Catalogo temporada de cosecha"
        url={rutaFrmCualidad}
      />
      ...
      <BuscadorBoton
        ...
        criterioFiltro={
          <RadioGrupCriterio
            ...
            itemList={ListCriterioTemporada}
          />
        }
      ...
      <TableCustom
        list={ItemTemporadaColumna}
        children={<TableBodyFrmTemporada List={data?.rows ? data?.rows : []} />}
      />
