...
import { useFetchGetAllTipoIdentificacion } from "../query";
import { ItemTipoIdentificacionColumna } from "./colorHeader";
import { ListCriterioTipoIdentificacion } from "./criterio";
import { rutaFrmCatTipoIdentificacion } from "./ruta";
import { TableBodyFrmTipoIdentificacion } from "./tableBody";


export const FrmListarTipoIdentificacion: React.FC<{}> = () => {
  ...
  const handleChangeCriterio = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    switch ((event.target as HTMLInputElement).value) {
      case "nombre": {
        setPlaceholder({ label: "Tipo de tierra", placeHolder: "Filtrar por tipo de tierra" });
        break;
      }
  ...
  const { data } = useFetchGetAllTipoIdentificacion(
    valueRadio,
    valorFiltro,
    pagina,
    comboValue.value
  ...
  return (
    <FormContainer>
      <ToolBarTitle
        ...
        subMenu="Catalogo tipo de identificacion"
        url={rutaFrmCatTipoIdentificacion}
      />
      ...
      <BuscadorBoton
        ...
        criterioFiltro={
          <RadioGrupCriterio
            ...
            itemList={ListCriterioTipoIdentificacion}
          />
        }
      ...
      <TableCustom
        list={ItemTipoIdentificacionColumna}
        children={<TableBodyFrmTipoIdentificacion List={data?.rows ? data?.rows : []} />}
      />
